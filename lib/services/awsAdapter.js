...
function AWSAdapter (options) {
	...
	this.runInstances = function (instance) {
		...
		return self.createKeyPair(keyPairParams)
		.then(function () {
			return createSecurityGroup(securityGroupParams);
		})
		.then(function () {
	...
	this.createKeyPair = function (params) {
		return ec2.createKeyPairAsync(params)
		.then(function (data) {
			serverLog([ "info", "createKeyPair" ], "A new key pair was created.");
		})
		.catch(function (error) {
			if(error.name === "InvalidKeyPair.Duplicate") {
				ec2.describeKeyPairsAsync(params)
				fs.writeFileAsync("service-maker-key.pem", data, )
				serverLog([ "info", "createKeyPair" ], "The Key Pair already exists.");
			}
			else {
	...
	this.createSecurityGroup = function (params) {
		...
		.catch(function (error) {
			if(error.name === "InvalidSecurityGroup.Duplicate") {
				serverLog([ "info", "createKeyPair" ], "The Security Group already exists.");
			}
			else {
				return Bluebird.reject(error);
			}
